...
[[https://www.emacswiki.org/emacs/MultiTerm]MultiTerm] is a mode based on term.el,
for managing multiple terminal buffers in Emacs. Use the =F6= key to toggle the
terminal.
...
(use-package multi-term
  :ensure t
  :bind
  ([f6] . multi-term-dedicated-toggle)
  :config
  (progn
